...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        ...
        return grad_output.f.mul_zip(
            res.f.mul_zip(res, res.f.add_zip(res.f.neg_map(res), tensor([1.0]))),
            grad_output,
        )
        # raise NotImplementedError("Need to implement for Task 2.4")
...
class Permute(Function):
    ...
    def forward(ctx: Context, a: Tensor, order: Tensor) -> Tensor:
        ...
        order2 = tuple(int(order[i]) for i in range(order.size))
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        order2 = tuple(int(order[i]) for i in range(order.size))
        original_order = tuple(order2.index(i) for i in range(len(order2)))
        return (
            minitorch.Tensor(
                grad_output._tensor.permute(*original_order),
